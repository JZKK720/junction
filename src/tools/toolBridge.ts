/**
 * ToolBridge — Dormant local-tool-execution scaffolding.
 *
 * ## Current status
 * `executeToolRequest()` is **NOT called anywhere**. The gateway agent runs tools
 * on the GATEWAY HOST (not inside the VSCode extension process). There is no
 * current protocol path to redirect a `tools.invoke` from the gateway to a
 * remote VSCode/IDE client. All tool events rendered in the chat panel reflect
 * server-side executions.
 *
 * ## Why keep this code
 * If a future protocol revision adds `tools.executeRemote` or similar dispatch,
 * this bridge is ready: it already validates workspace boundaries, handles
 * destructive-operation confirmation, opens documents to apply edits, and
 * creates named terminals. The code is nontrivial enough that discarding it
 * would waste useful scaffolding.
 *
 * ## What would need to change before this is usable
 * 1. The gateway would need a protocol message that routes a `tools.invoke`
 *    (or equivalent) to a connected client by `client.id`.
 * 2. The VSCode extension would need to subscribe to that message type and
 *    call `new ToolBridge().executeToolRequest(req)`.
 * 3. Tool-result messages would need a client→gateway return path (existing
 *    `sendRequest` / `res` plumbing already supports that direction).
 *
 * Until then, this module is **retained but dormant** — no callers, no imports
 * from startup paths, zero runtime cost.
 */
