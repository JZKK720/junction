...
export function registerShowLogsCommand(
    ...
    const handler = async () => {
            ...
            if (logFilePath) {
                ...
                const result = await vscode.window.showInformationMessage(
                    `Junction logs are available at: ${logFilePath}`,
                    openFile
                );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('junction.showLogs', handler),
        vscode.commands.registerCommand('openclaw.showLogs', handler),
    );
}
